...
/**
 * Transcribe an already-buffered recording without uploading it to storage
 * first. This keeps the existing URL-based helper intact for other callers,
 * while shortening the voice question critical path by two network operations.
 */
  ...
  try {
    if (!ENV.forgeApiUrl || !ENV.forgeApiKey) {
      return {
        error: "Voice transcription service is not configured",
        code: "SERVICE_ERROR",
        details: !ENV.forgeApiUrl
          ? "BUILT_IN_FORGE_API_URL is not set"
